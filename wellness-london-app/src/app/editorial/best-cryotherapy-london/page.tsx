...
function CryotherapyVenueCard({ facility, index }: { facility: ServiceDirectoryFacility; index: number }) {
  ...
  return (
    <article className="group grid gap-5 border-t border-[#29241d]/18 py-7 md:grid-cols-[0.18fr_0.28fr_0.54fr] md:items-start">
      ...
      <div>
        <p className="mb-3 text-xs uppercase tracking-[0.24em] text-[#8d7d67]">{getLocationLine(facility) || "London"}</p>
        ...
        <dl className="mt-5 grid gap-3 border-y border-[#d8cebf]/80 py-4 text-sm leading-6 text-[#5f574c] sm:grid-cols-2">
          <div><dt className="text-xs uppercase tracking-[0.2em] text-[#8d7d67]">Services</dt><dd>{services}</dd></div>
          <div><dt className="text-xs uppercase tracking-[0.2em] text-[#8d7d67]">Location</dt><dd>{getLocationLine(facility) || "London"}</dd></div>
        </dl>
  ...
  return (
    <main className="min-h-screen bg-[#f4efe6] text-[#29241d]">
      ...
      <article>
          ...
          <div className="relative mx-auto grid max-w-[1500px] lg:min-h-[720px] lg:grid-cols-[0.92fr_1.08fr]">
            <div className="flex flex-col justify-between px-5 py-8 sm:px-8 sm:py-10 lg:px-12 lg:py-12">
              ...
              <div className="mt-24 max-w-2xl lg:mt-0">
                <p className="mb-6 text-xs uppercase tracking-[0.28em] text-[#cbbda8]">Cryotherapy Guide</p>
              ...
              <div className="mt-14 grid gap-4 border-t border-[#fbf8f1]/16 pt-5 text-sm leading-6 text-[#fbf8f1]/68 sm:grid-cols-3"><div><p className="text-xs uppercase tracking-[0.22em] text-[#fbf8f1]/42">Focus</p><p>Cold-air recovery</p></div><div><p className="text-xs uppercase tracking-[0.22em] text-[#fbf8f1]/42">Format</p><p>Evidence-led guide</p></div><div><p className="text-xs uppercase tracking-[0.22em] text-[#fbf8f1]/42">Source</p><p>{cryotherapyFacilities.length} live listings</p></div></div>
            </div>
        ...
        <section className="px-5 py-14 sm:px-6 sm:py-20"><div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.32fr_0.68fr]"><aside><p className="text-xs uppercase tracking-[0.24em] text-[#8d7d67]">What is cryotherapy?</p></aside><div className="max-w-3xl space-y-6 text-base leading-8 text-[#5f574c] sm:text-lg sm:leading-9"><p className="font-serif text-3xl leading-[1.16] tracking-[-0.02em] text-[#29241d] sm:text-4xl">Cryotherapy is a short, supervised cold-air exposure used by many people as a recovery tool.</p><p>Whole-body cryotherapy usually means standing in a specialist chamber or cabin cooled far below normal environmental temperatures, commonly for only a few minutes. Unlike a cold plunge, the stressor is dry cold air rather than water immersion, so the session can feel cleaner, shorter and more controlled.</p><p>People commonly use cryotherapy for perceived soreness, post-training recovery, competition blocks, fatigue management and as a structured cold-exposure appointment.</p></div></div></section>

        <section className="px-5 py-10 sm:px-6 sm:py-16"><div className="mx-auto max-w-6xl"><div className="mb-6 grid gap-5 lg:grid-cols-[0.32fr_0.68fr]"><p className="text-xs uppercase tracking-[0.24em] text-[#8d7d67]">Featured venues</p><h2 className="font-serif text-[3rem] leading-[0.92] tracking-[-0.03em] sm:text-6xl">Compare cryotherapy venues.</h2></div>{visibleVenues.map((facility, index) => <CryotherapyVenueCard key={facility.slug} facility={facility} index={index} />)}</div></section>

        <section className="px-5 py-10 sm:px-6 sm:py-16"><div className="mx-auto max-w-6xl"><div className="mb-8 grid gap-5 lg:grid-cols-[0.32fr_0.68fr] lg:items-end"><p className="text-xs uppercase tracking-[0.24em] text-[#8d7d67]">Cryotherapy vs cold plunge</p><h2 className="font-serif text-[3rem] leading-[0.92] tracking-[-0.03em] sm:text-6xl">Two forms of cold, different rituals.</h2></div><div className="overflow-hidden rounded-[1.4rem] border border-[#d8cebf]"><div className="grid grid-cols-3 bg-[#29241d] p-4 text-sm uppercase tracking-[0.18em] text-[#fbf8f1]"><span></span><span>Cryotherapy</span><span>Cold Plunge</span></div>{comparisonRows.map((row) => <div key={row.label} className="grid grid-cols-3 gap-4 border-t border-[#d8cebf] bg-[#fbf8f1] p-4 text-sm leading-6 text-[#5f574c]"><strong className="font-serif text-xl font-normal text-[#29241d]">{row.label}</strong><span>{row.cryotherapy}</span><span>{row.coldPlunge}</span></div>)}</div></div></section>

        <section className="px-5 py-10 sm:px-6 sm:py-16"><div className="mx-auto max-w-6xl rounded-[2rem] bg-[#29241d] p-6 text-[#fbf8f1] sm:p-8"><p className="text-xs uppercase tracking-[0.24em] text-[#cbbda8]">What the research says</p><h2 className="mt-4 max-w-3xl font-serif text-[3rem] leading-[0.92] tracking-[-0.03em] sm:text-6xl">Useful signals, not miracle claims.</h2><div className="mt-10 grid gap-px overflow-hidden rounded-[1.2rem] bg-[#fbf8f1]/14 lg:grid-cols-3">{researchCards.map((card) => <div key={card.label} className="bg-[#342f27] p-6"><p className="text-xs uppercase tracking-[0.22em] text-[#cbbda8]">{card.label}</p><h3 className="mt-8 font-serif text-3xl font-normal leading-[1]">{card.title}</h3><ul className="mt-5 space-y-3 text-sm leading-6 text-[#fbf8f1]/72">{card.points.map((point) => <li key={point}>— {point}</li>)}</ul></div>)}</div></div></section>
        ...
        <section className="px-5 pb-10 sm:px-6 sm:pb-16"><div className="mx-auto max-w-6xl"><p className="mb-6 text-xs uppercase tracking-[0.24em] text-[#8d7d67]">FAQ</p><div className="divide-y divide-[#29241d]/18 border-y border-[#29241d]/18">{faqs.map((faq) => <details key={faq.question} className="group py-5"><summary className="flex cursor-pointer list-none justify-between gap-6 font-serif text-2xl leading-[1.1]"><span>{faq.question}</span><span className="transition group-open:rotate-45">+</span></summary><p className="mt-4 max-w-3xl text-base leading-8 text-[#5f574c]">{faq.answer}</p></details>)}</div></div></section>

        <section className="px-5 pb-16 pt-6 sm:px-6 sm:pb-24"><div className="mx-auto rounded-[2rem] bg-[#29241d] p-6 text-[#fbf8f1] sm:p-8"><p className="text-xs uppercase tracking-[0.24em] text-[#fbf8f1]/58">Continue exploring</p><div className="mt-6 flex flex-wrap gap-3"><LinkPill href="/cryotherapy-london">Cryotherapy service page</LinkPill><LinkPill href="/contrast-therapy-london">Contrast therapy</LinkPill><LinkPill href="/recovery-london">Recovery</LinkPill><LinkPill href="/collections/best-contrast-therapy-london">Best sauna + cold plunge</LinkPill><LinkPill href="/collections/best-sauna-london">Best saunas in London</LinkPill></div></div></section>
      </article>
    </main>
  );
}
...
function EditorialSplitSection({ eyebrow, title, items, dark = false }: { eyebrow: string; title: string; items: { title: string; text: string }[]; dark?: boolean }) {
  return <section className="px-5 py-10 sm:px-6 sm:py-16"><div className={`mx-auto max-w-6xl ${dark ? "bg-[#fbf8f1]" : ""}`}><div className="mb-8 grid gap-5 border-t border-[#29241d]/18 pt-8 lg:grid-cols-[0.32fr_0.68fr]"><p className="text-xs uppercase tracking-[0.24em] text-[#8d7d67]">{eyebrow}</p><h2 className="font-serif text-[3rem] leading-[0.92] tracking-[-0.03em] sm:text-6xl">{title}</h2></div><div className="grid gap-px overflow-hidden bg-[#29241d]/16 md:grid-cols-3">{items.map((item) => <div key={item.title} className="bg-[#f4efe6] p-6"><h3 className="font-serif text-3xl font-normal leading-[1] tracking-[-0.02em]">{item.title}</h3><p className="mt-5 text-sm leading-7 text-[#5f574c]">{item.text}</p></div>)}</div></div></section>;
}
