...
export const metadata: Metadata = {
  title: northLondonGuide.title,
  description: northLondonGuide.description,
  alternates: {
...
export default function NorthLondonWellnessPage() {
  const faqSchema = {
    ...
    "@type": "FAQPage",
    mainEntity: northLondonGuide.faqs.map((faq) => ({
      "@type": "Question",
      name: faq.question,
      acceptedAnswer: {
        "@type": "Answer",
  ...
  return (
    <main className="min-h-screen bg-[#f4efe6] text-[#29241d]">
      <AnalyticsPageView eventName="location_page_view" properties={{ location: "north-london", page_path: "/north-london-wellness" }} />
        ...
        <div className="mx-auto max-w-6xl border-y border-[#d8cebf]/70 py-10 sm:py-14">
          <div className="grid gap-10 md:grid-cols-[0.9fr_1.1fr] md:items-start">
            <div>
              ...
              <h1 className="font-serif text-5xl font-normal leading-[0.98] tracking-normal sm:text-6xl md:text-8xl">{northLondonGuide.title}</h1>
            </div>
            <div className="space-y-6 text-base leading-8 text-[#5f574c]">
              {northLondonGuide.intro.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#fbf8f1] px-5 py-14 sm:px-6 sm:py-20">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 max-w-3xl">
            ...
            <h2 className="font-serif text-4xl font-normal leading-tight sm:text-5xl">North London wellness areas.</h2>
          </div>
          <div className="flex flex-wrap gap-3">
            {northLondonGuide.areas.map((area) => <span key={area} className="bg-[#f4efe6] px-4 py-2 text-sm text-[#5f574c]">{area}</span>)}
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-16 sm:px-6 sm:py-24">
        <div className="mx-auto max-w-3xl">
          <h2 className="mb-8 text-2xl font-medium tracking-normal sm:text-3xl md:text-4xl">North London Wellness FAQs</h2>
          <div className="space-y-7 sm:space-y-8">
            {northLondonGuide.faqs.map((faq) => (
