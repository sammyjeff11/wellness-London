...
export default async function NeighbourhoodsPage() {
  ...
  const regionGroups = londonRegions
    .map((region) => ({
      ...
      neighbourhoods: availableNeighbourhoods.filter(
        ({ page }) => page.region === region.name,
      ),
    }))
    ...
    <main className="min-h-screen bg-[#f4efe6] text-[#29241d]">
      ...
      <section className="px-5 py-8 sm:px-6 sm:py-10">
        <div className="mx-auto max-w-6xl">
          <nav
            ...
            className="mb-10 flex items-center gap-2 text-sm text-[#6f6048]"
          >
          ...
          <h1 className="max-w-4xl font-serif text-[3.4rem] font-normal leading-[0.92] tracking-[-0.05em] sm:text-6xl">
            Wellness across London.
          </h1>
          <p className="mt-7 max-w-3xl text-base leading-8 text-[#5f574c] sm:text-lg">
            Begin with a broad London region, then move into a neighbourhood
            when proximity and the character of the area matter more than one
            specific service.
          </p>
        </div>
      </section>
      ...
      >
        <div className="mx-auto max-w-6xl">
          ...
          >
            Choose an area of London.
          </h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-[#5f574c] sm:text-base">
            Each area gives you the wider picture first. Where the directory has
            enough verified local coverage, continue into a neighbourhood guide.
          </p>
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {regionGroups.map(
              ({ region, facilities: regionFacilities, neighbourhoods }) => (
                <article
                  key={region.href}
                  className="surface-paper rounded-[1rem] p-6 sm:p-7"
                >
                  <p className="text-[10px] uppercase tracking-[0.2em] text-[#8d7d67]">
                    {regionFacilities.length}{" "}
                    {regionFacilities.length === 1 ? "venue" : "venues"}
                  </p>
                  <h3 className="mt-4 font-serif text-4xl font-normal leading-none tracking-[-0.04em]">
                    {region.name}
                  </h3>
                  <p className="mt-4 max-w-xl text-sm leading-7 text-[#5f574c]">
                    {region.copy}
                  </p>
                  <Link
                    href={region.href}
                  ...
                  {neighbourhoods.length > 0 ? (
                    <div className="mt-7 border-t border-[#d8cebf] pt-5">
                      <p className="text-[10px] uppercase tracking-[0.2em] text-[#8d7d67]">
                        Neighbourhoods with listings
                      </p>
                      <div className="mt-3 flex flex-wrap gap-2">
                        {neighbourhoods.map(
                          ({ page, facilities: localFacilities }) => (
                            <Link
                              key={page.slug}
                              href={page.href}
                              className="rounded-full border border-[#d8cebf] px-3 py-2 text-sm transition hover:bg-[#eee7da]"
                            >
      ...
      >
        <div className="mx-auto max-w-6xl">
          ...
          <h2
            ...
            className="font-serif text-4xl font-normal leading-none tracking-[-0.04em] sm:text-5xl"
          >
            Neighbourhoods with verified listings.
          </h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-[#5f574c] sm:text-base">
            These are the neighbourhoods where Well+ currently has at least one
            published venue. Empty or draft-only locations are not shown.
          </p>
          <div className="mt-8 grid gap-3 md:grid-cols-2">
            {availableNeighbourhoods.map(
              ({ page: area, facilities: localFacilities }) => {
                const count = localFacilities.length;
                return (
                  <Link
                    key={area.slug}
                    href={area.href}
                    className="surface-paper group grid gap-5 rounded-[1rem] p-6 transition hover:-translate-y-0.5 hover:bg-[#f5f0e7] sm:grid-cols-[0.72fr_1.28fr] sm:p-7"
                  >
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.2em] text-[#8d7d67]">
                        {area.eyebrow}
                      ...
                      <p className="mt-4 text-sm underline underline-offset-4">
                        {count > 0
                          ? `${count} matched ${count === 1 ? "venue" : "venues"}`
                          : "Read area guide"}{" "}
                        →
                      </p>
                    </div>
