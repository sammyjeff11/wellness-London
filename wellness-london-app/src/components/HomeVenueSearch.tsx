...
function getResultLocation(facility: ServiceDirectoryFacility) {
  return [
    ...
    facility.areaOfLondon || facility.areaGroup,
  ]
    .filter(Boolean)
...
export default function HomeVenueSearch({ facilities }: HomeVenueSearchProps) {
  ...
  const results = useMemo(() => {
    ...
    return facilities
      ...
      .sort(
        (a, b) =>
          rankVenueSearch(b, trimmedQuery) - rankVenueSearch(a, trimmedQuery),
      )
      .slice(0, 5);
    ...
    <div className="relative mt-6 max-w-2xl sm:mt-8">
      ...
      <form
        ...
        onSubmit={() =>
          ...
          trackEvent(
            results.length
              ? "venue_search_submitted"
              : "venue_search_no_results",
            { search_length: trimmedQuery.length, source: "homepage" },
          )
        }
      >
        <input
      ...
      {trimmedQuery ? (
        <div
          ...
          className="mt-2 overflow-hidden rounded-[1rem] border border-[#d8cebf] bg-[#fbf8f1] p-2 text-[#29241d] shadow-[0_24px_60px_rgba(0,0,0,0.28)]"
        >
          {results.length > 0 ? (
            ...
            <div className="px-3 py-3">
              ...
              <p className="mt-1 text-xs leading-5 text-[#5f574c]">
                Try a venue, service or London neighbourhood.
              </p>
            </div>
          )}
          <Link
            href={`/explore?q=${encodeURIComponent(trimmedQuery)}`}
            className="mt-2 block rounded-lg px-3 py-3 text-sm font-medium underline underline-offset-4"
          >
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {popularLinks.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className="inline-flex min-h-11 items-center rounded-full border border-[#fbf8f1]/24 bg-black/10 px-3 py-1.5 text-xs text-[#fbf8f1]/88 transition hover:border-[#fbf8f1]/60 hover:bg-[#fbf8f1] hover:text-[#29241d]"
          >
            {link.label}
          </Link>
        ))}
