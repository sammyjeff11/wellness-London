...
function QuickLinks({ title, links }: { title: string; links: { href: string; label: string }[] }) {
  return (
    <div>
      <p className="mb-2 text-[10px] uppercase leading-4 tracking-[0.2em] text-[#8d7d67]">{title}</p>
      <div className="flex flex-wrap gap-2">
        {links.map((link) => (
          <Link
            key={`${title}-${link.href}-${link.label}`}
            href={link.href}
            className="rounded-full border border-[#cfc1ad]/80 bg-[#fbf8f1]/62 px-3 py-1.5 text-xs leading-5 text-[#29241d] transition hover:border-[#8d7d67] hover:bg-[#fbf8f1]"
          >
            {link.label}
          </Link>
        ))}
...
export default function HomeVenueSearch({ facilities }: HomeVenueSearchProps) {
  ...
  return (
    <section className="px-5 py-5 sm:px-6 sm:py-9">
      <div className="editorial-shell overflow-hidden rounded-[1.35rem] border border-[#d8cebf]/80 bg-[#eee6d8] shadow-[0_18px_50px_rgba(41,36,29,0.045)] sm:rounded-[1.55rem]">
        <div className="grid gap-5 p-5 sm:p-7 md:grid-cols-[0.82fr_1.18fr] md:gap-8 md:p-8">
          <div>
            <p className="editorial-eyebrow mb-3">Well+ directory</p>
            <h2 className="font-serif text-[2.1rem] font-normal leading-[1.02] tracking-[-0.045em] sm:text-4xl md:text-5xl">
              Search by name, area or service.
            </h2>
            <p className="mt-3 max-w-md text-sm leading-6 text-[#5f574c] sm:mt-4 sm:text-base sm:leading-7">
          ...
          <div className="space-y-4">
            <label className="block">
              ...
              <input
                ...
                className="w-full rounded-full border border-[#cfc1ad] bg-[#fbf8f1] px-5 py-3 text-[15px] leading-6 text-[#29241d] shadow-[inset_0_1px_0_rgba(255,255,255,0.75)] outline-none transition placeholder:text-[#8d7d67] focus:border-[#6f6048] focus:ring-2 focus:ring-[#d8cebf] sm:py-3.5"
              />
            </label>

            {trimmedQuery ? (
              <div className="rounded-[1rem] border border-[#d8cebf]/80 bg-[#fbf8f1]/78 p-2">
                {results.length > 0 ? (
            ...
            <div className="grid gap-3 border-t border-[#d8cebf]/80 pt-4 sm:grid-cols-3">
