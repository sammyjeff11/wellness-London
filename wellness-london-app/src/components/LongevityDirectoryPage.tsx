...
function profileClinic(facility: AirtableFacility): ClinicProfile {
  ...
  } else if (containsAny(text, ["mri", "ct scan", "medical imaging", "full body scan", "cancer screening"])) {
    need = "imaging";
    clinicType = "Imaging and screening provider";
    bestFor = "Advanced screening and structural investigation";
  } else if (containsAny(text, ["vo2 max", "vo₂ max", "dexa", "resting metabolic rate", "performance testing"])) {
    need = "performance";
  ...
  if (containsAny(text, ["doctor-led", "physician-led", "medical director", "doctor consultation", "consultant physician"])) {
    oversight = "doctor";
    oversightLabel = "Doctor-led";
  } else if (containsAny(text, ["clinician-led", "clinical team", "nurse-led", "physiologist"])) {
    oversight = "clinician";
    oversightLabel = "Clinician-led";
  }
      ...
      <section id="how-to-compare" className="px-5 py-10 sm:px-6 sm:py-14">
        <div className="mx-auto grid max-w-6xl gap-8 md:grid-cols-[0.78fr_1.22fr]">
          ...
          <div className="space-y-5 text-base leading-8 text-[#5f574c]">
            <p>Longevity clinics use medical expertise and objective testing to build a clearer picture of current health. The focus is on establishing a baseline, identifying potential risk and interpreting results before recommending a plan.</p>
            <p>This directory only includes providers with a meaningful diagnostics, screening or clinician-led assessment offer. Sauna, cold therapy, red light, HBOT or IV therapy alone do not qualify a venue.</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#29241d] px-5 py-14 text-[#fbf8f1] sm:px-6 md:py-20">
        <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-[0.8fr_1.2fr]">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {["Who reviews and explains the results?", "Is a written report included?", "Will you receive a personalised plan?", "Are follow-up testing and referrals available?"].map((item) => <div key={item} className="border border-[#fbf8f1]/14 p-5 text-sm leading-7 text-[#fbf8f1]/78">{item}</div>)}
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-12 sm:px-6 md:py-20">
        <div className="mx-auto grid max-w-6xl gap-8 md:grid-cols-[0.75fr_1.25fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Link href="/editorial" className="border border-[#d8cebf] bg-[#f4efe6] p-6 transition hover:bg-[#eee7da]"><h3 className="text-xl font-medium">Longevity testing explained</h3><p className="mt-3 text-sm leading-7 text-[#5f574c]">Understand screening, diagnostics, evidence and what different tests can—and cannot—tell you.</p></Link>
            <Link href="/editorial-standards" className="border border-[#d8cebf] bg-[#f4efe6] p-6 transition hover:bg-[#eee7da]"><h3 className="text-xl font-medium">How Well+ handles health claims</h3><p className="mt-3 text-sm leading-7 text-[#5f574c]">How we approach evidence, uncertainty and medical-adjacent services.</p></Link>
          </div>
        </div>
      </section>
    </main>
