...
import {
  getMatchedPillarServices,
  prioritiseServicesForPillar,
  type ServicePillar,
  ...
  const featuredFacilities = dedupeFacilities(facilities.map(toDirectoryFacility))
    ...
    .map((facility) => {
      ...
      return {
        ...
        prioritisedService: matchedPillarServices[0]?.name,
      };
    });
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {featuredFacilities.length > 0 ? (
          <div className="grid gap-10 sm:gap-12 md:grid-cols-2 md:gap-8 xl:grid-cols-3">
            {featuredFacilities.map(({ facility, prioritisedService }) => (
              <FacilityCard
                key={facility.slug}
                facility={facility}
                source={`${pillar.slug}_pillar`}
                prioritisedService={prioritisedService}
              />
            ))}
          </div>
        ) : (
          <div className="border border-[#d8cebf] bg-[#f4efe6] p-8 text-sm leading-7 text-[#5f574c]">Venue collections are still being expanded for this pillar.</div>
        )}
      </div>
    </section>
  );
