...
import {
  getFacilitiesForActivity,
  type ActivityPageConfig,
} from "@/lib/activity-pages";
...
function canonicalRelatedHref(href: string) {
  if (href === "/collections/best-sauna-london")
    return "/editorial/best-saunas-london";
  return href;
}

function ColdContrastIntentGuide({
  slug,
}: {
  slug: ActivityPageConfig["slug"];
}) {
  if (slug !== "cold-plunge-london" && slug !== "contrast-therapy-london")
    return null;
  ...
  return (
    <section
      className="surface-band-stone px-5 py-10 sm:px-6 sm:py-14"
      aria-labelledby={`${slug}-intent-heading`}
    >
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-6 lg:grid-cols-[0.72fr_1.28fr] lg:items-start">
          <div>
            ...
            <h2
              id={`${slug}-intent-heading`}
              className="font-serif text-4xl font-normal leading-none tracking-[-0.04em] sm:text-5xl"
            >
              {isColdPlunge
                ? "Cold plunge is the cold-water route."
                : "Contrast therapy is the hot-and-cold route."}
            </h2>
          </div>

          <div className="grid gap-4 md:grid-cols-2">
            <article
              className={`rounded-[1.2rem] border p-6 ${isColdPlunge ? "border-[#29241d] bg-[#fbf8f1]" : "border-[#d8cebf] bg-[#fbf8f1]/70"}`}
            >
              ...
              <p className="mt-4 text-sm leading-7 text-[#5f574c]">
                Use this route when the main thing you want is an ice bath, cold
                tub, plunge pool or other cold-water session — whether
                standalone or available alongside other facilities.
              </p>
              <Link
            ...
            >
              <p className="text-[10px] uppercase tracking-[0.2em] text-[#8d7d67]">
                Contrast therapy / sauna and plunge
              </p>
              <h3 className="mt-4 font-serif text-3xl leading-none tracking-[-0.035em]">
                Heat and cold together
              </h3>
              <p className="mt-4 text-sm leading-7 text-[#5f574c]">
                Use this route when the experience itself is the alternation
                between sauna or heat and cold-water immersion, including guided
                rounds and full thermal circuits.
              </p>
              <Link
        ...
        <div className="mt-5 flex flex-wrap gap-3 border-t border-[#d8cebf]/80 pt-5 text-sm">
          <Link
            ...
            className="rounded-full border border-[#d8cebf] px-4 py-2 transition hover:bg-[#fbf8f1]"
          >
...
export default async function ActivityServicePage({
  activity,
}: ActivityServicePageProps) {
  const facilities = await getFacilities();
  const activityFacilities = getFacilitiesForActivity(facilities, activity);
  const directoryFacilities = dedupeFacilities(
    activityFacilities.map(toDirectoryFacility),
  );
  const heroImage = activityFacilities.find(
    (facility) => facility.images.length > 0,
  )?.images[0];
  const relatedLinks = [
    ...activity.related.map((link) => ({
      ...link,
      href: canonicalRelatedHref(link.href),
    })),
  ...
  return (
    <main className="min-h-screen bg-[#f4efe6] text-[#29241d]">
      <AnalyticsPageView
        ...
        properties={{ service_type: serviceType, page_path: activity.href }}
      />
      ...
      <section className="px-4 pt-4 sm:px-5 md:px-8 md:pt-8">
        <div className="relative mx-auto flex min-h-[20rem] max-w-[1400px] items-end overflow-hidden bg-[#8f806b] px-5 py-10 sm:min-h-[22rem] sm:px-6 sm:py-12 md:px-14 md:py-10">
          {heroImage ? (
            <SafeImage
              src={heroImage.url}
              alt={`${activity.label} venue in London`}
              fill
              priority
              sizes="100vw"
      ...
      <ServiceDirectorySection
        ...
        prioritisedService={activity.label}
        emptyTitle={`No ${activity.label.toLowerCase()} listings yet`}
        emptyText={`No published ${activity.label.toLowerCase()} venues currently meet the directory criteria.`}
      />

      <ServiceIntroSection
        eyebrow={`${activity.label} guide`}
        title="Understand the experience before you book."
        paragraphs={[activity.description]}
      />
      <ColdContrastIntentGuide slug={activity.slug} />
      {activity.bestFor.length > 0 ? (
        <ServiceInsightSection
          eyebrow="Compare the booking"
          panels={activity.bestFor}
        />
      ) : null}
      <ServiceGuidanceSection
        eyebrow="Compare the formats"
        title={`What to check before booking ${activity.label.toLowerCase()}`}
        points={activity.guidance}
      />

      <ServiceGuidanceSection
        eyebrow="The session"
        title={`What to expect from ${activity.label.toLowerCase()} in London`}
        points={activity.whatToExpect}
      />
      <ServiceEvidenceSection notes={activity.evidenceNotes} />

      <section className="px-5 py-12 sm:px-6 md:py-10">
        <div className="mx-auto max-w-6xl">
          <NewsletterSignup
      ...
      <ServiceFaqSection
        title={`${activity.label} London FAQs`}
        faqs={activity.faqs}
      />
    </main>
  );
