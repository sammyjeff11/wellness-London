...
export function ServiceIntroSection({ eyebrow, title, paragraphs }: ServiceIntroSectionProps) {
  return (
    <section className="editorial-section border-y border-[#cbbda9] bg-[#fbf8f1] px-5 sm:px-6">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-9 md:grid-cols-[0.95fr_1.05fr] md:items-start">
          <div>
            <p className="mb-4 text-xs uppercase tracking-[0.24em] text-[#6f6048]">{eyebrow}</p>
...
export function ServiceDirectorySection({ facilities, serviceType, emptyTitle, emptyText, prioritisedService }: ServiceDirectorySectionProps) {
  return (
    <section id="venues" className="editorial-section scroll-mt-28 bg-[#f4efe6] px-5 sm:px-6">
      <div className="mx-auto max-w-6xl">
...
export function ServiceGuidanceSection({ title, points }: ServiceGuidanceSectionProps) {
  return (
    <section className="editorial-section border-y border-[#cbbda9] bg-[#29241d] px-5 text-[#fbf8f1] sm:px-6">
      <div className="mx-auto max-w-6xl">
        <p className="mb-4 text-xs uppercase tracking-[0.24em] text-[#d8cebf]">How to choose</p>
        <h2 className="mb-10 max-w-3xl font-serif text-4xl font-normal leading-tight sm:text-5xl">{title}</h2>
        <div className="grid gap-8 sm:gap-10 md:grid-cols-4">
          {points.map((point) => (
            <article key={point.title} className="border-t border-[#fbf8f1]/22 pt-5">
              <h3 className="mb-3 text-base uppercase tracking-[0.14em] text-[#fbf8f1]">{point.title}</h3>
              <p className="text-[15px] leading-7 text-[#fbf8f1]/76">{point.text}</p>
            </article>
          ))}
        </div>
      </div>
...
export function ServiceEvidenceSection({ title = "What the evidence suggests", notes = [] }: ServiceEvidenceSectionProps) {
  ...
  return (
    <section className="editorial-section border-b border-[#cbbda9] bg-[#fbf8f1] px-5 sm:px-6">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 max-w-3xl">
          <p className="mb-4 text-xs uppercase tracking-[0.24em] text-[#6f6048]">Science-informed guide</p>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {notes.map((note) => (
            <article key={note.title} className="editorial-card bg-[#f4efe6] p-5 sm:p-6">
              <h3 className="mb-3 text-xl font-medium tracking-normal text-[#29241d]">{note.title}</h3>
              <p className="text-[15px] leading-7 text-[#5f574c]">{note.text}</p>
            </article>
          ))}
        </div>
      </div>
...
export function ServiceInsightSection({ eyebrow = "Before you book", panels }: ServiceInsightSectionProps) {
  return (
    <section className="editorial-section border-b border-[#cbbda9] bg-[#eee7da] px-5 sm:px-6">
      <div className="mx-auto max-w-6xl">
        <p className="mb-4 text-xs uppercase tracking-[0.24em] text-[#6f6048]">{eyebrow}</p>
        <div className="grid gap-5 md:grid-cols-3">
          {panels.map((panel) => (
            <article key={panel.title} className="editorial-card bg-[#fbf8f1] p-6 shadow-[0_20px_50px_rgba(41,36,29,0.045)] sm:p-7">
              <h3 className="mb-4 text-xl font-medium tracking-normal">{panel.title}</h3>
              <p className="text-[15px] leading-7 text-[#5f574c]">{panel.text}</p>
            </article>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {links.map((link) => {
            ...
            return (
              <Link key={href} href={href} className="editorial-card group block bg-[#f4efe6] p-6 transition hover:bg-[#eee7da] sm:p-7">
                ...
                <p className="text-[15px] leading-7 text-[#5f574c]">{link.text}</p>
...
export function ServiceFaqSection({ title, faqs }: ServiceFaqSectionProps) {
  return (
    <section className="editorial-section border-t border-[#cbbda9] bg-[#eee7da] px-5 sm:px-6">
      <div className="editorial-card mx-auto max-w-3xl bg-[#fbf8f1] p-6 shadow-[0_20px_60px_rgba(41,36,29,0.05)] sm:p-8 md:p-10">
        ...
        <div className="space-y-7 sm:space-y-8">
          {faqs.map((faq) => (
            <article key={faq.question} className="border-t border-[#d8cebf] pt-6">
              ...
              <p className="text-[15px] leading-7 text-[#5f574c]">{faq.answer}</p>
            </article>
          ))}
        </div>
      </div>
