      ...
      <div className="relative overflow-hidden rounded-[1.15rem] border border-[#d8cebf]/75 bg-[#fbf8f1]/72 p-2 shadow-[0_16px_40px_rgba(41,36,29,0.04)] sm:rounded-[1.35rem] sm:p-3">
        <div className="grid gap-2 md:grid-cols-[1.35fr_0.65fr]">
          <button
            ...
            className="group relative aspect-[4/3] overflow-hidden rounded-[0.95rem] bg-[#d8cebf] text-left focus:outline-none focus:ring-2 focus:ring-[#6f6048] md:aspect-[5/4]"
            ...
            <div className="hidden gap-2 md:grid">
              {secondaryImages.slice(0, 2).map((image, index) => (
                <button
                  ...
                  className="group relative overflow-hidden rounded-[0.95rem] bg-[#d8cebf] text-left focus:outline-none focus:ring-2 focus:ring-[#6f6048]"
                  aria-label={`Open ${venueName} photo ${index + 2}`}
                >
                  <SafeImage
                    src={image.url}
          ...
          <div className="mt-2 flex snap-x snap-mandatory gap-2 overflow-x-auto pb-1 md:hidden">
            {secondaryImages.map((image, index) => (
              <button
                ...
                type="button"
                onClick={() => openLibrary(index + 1)}
                className="relative aspect-[4/3] min-w-[42%] snap-start overflow-hidden rounded-[0.8rem] bg-[#d8cebf] focus:outline-none focus:ring-2 focus:ring-[#6f6048]"
                aria-label={`Open ${venueName} photo ${index + 2}`}
              >
                <SafeImage
                  src={image.url}
        ...
        {galleryImages.length > 1 ? (
          <button
            type="button"
            onClick={() => openLibrary(0)}
            className="absolute bottom-4 right-4 rounded-full bg-[#fbf8f1]/95 px-4 py-2 text-xs font-medium text-[#29241d] shadow-[0_8px_22px_rgba(41,36,29,0.14)] transition hover:bg-white focus:outline-none focus:ring-2 focus:ring-[#6f6048]"
          >
            View photos
          </button>
        ) : null}
          ...
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 sm:px-6">
              ...
              <button
                ...
                className="rounded-full border border-white/20 px-4 py-2 text-sm transition hover:bg-white hover:text-[#29241d] focus:outline-none focus:ring-2 focus:ring-white/70"
              >
                Close
              </button>
            </div>
            ...
            <div className="relative flex min-h-0 flex-1 items-center justify-center px-4 py-4 sm:px-6">
              {galleryImages.length > 1 ? (
                <button
                  ...
                  className="absolute left-3 top-1/2 z-10 hidden -translate-y-1/2 rounded-full bg-[#fbf8f1]/92 px-4 py-3 text-[#29241d] shadow-lg transition hover:bg-white focus:outline-none focus:ring-2 focus:ring-white/70 sm:block"
              ...
              <div className="relative h-full max-h-[68vh] w-full max-w-5xl overflow-hidden rounded-[1rem] bg-black/30 sm:max-h-[72vh]">
                <SafeImage
                  src={activeImage.url}
                  alt={activeImage.filename || `${venueName} photo ${activeIndex + 1}`}
                  fill
                  sizes="100vw"
              ...
              {galleryImages.length > 1 ? (
                <button
                  ...
                  className="absolute right-3 top-1/2 z-10 hidden -translate-y-1/2 rounded-full bg-[#fbf8f1]/92 px-4 py-3 text-[#29241d] shadow-lg transition hover:bg-white focus:outline-none focus:ring-2 focus:ring-white/70 sm:block"
              ...
              <div className="border-t border-white/10 px-4 py-3 sm:px-6">
                <div className="flex snap-x snap-mandatory gap-2 overflow-x-auto pb-1">
                  {galleryImages.map((image, index) => {
                    ...
                    return (
                      <button
                        key={`${image.url}-library-${index}`}
                        type="button"
                        onClick={() => setActiveIndex(index)}
                        className={`relative aspect-[4/3] min-w-[7rem] snap-start overflow-hidden rounded-[0.7rem] bg-[#d8cebf] transition focus:outline-none focus:ring-2 focus:ring-white/70 sm:min-w-[8.5rem] ${
                          isActive ? "ring-2 ring-[#fbf8f1]" : "opacity-65 hover:opacity-100"
                        }`}
